...
y1 <- stan_lm(lm(cosp ~ treatment + bill + leg, data = subset(df , office.size > 1 & passed==0), weights=ipw), refresh = FALSE, prior = NULL)

y2 <- stan_lm(lm(cosp ~ treatment + bill + leg, data = subset(df , office.size > 1 & passed==1), weights=ipw), refresh = FALSE, prior = NULL)
...
y_final <- y_final %>%
  mutate(Combined = round(`treatment1.1` * 100, 2),
         ...
         `Reached Floor?` = c("Bills that failed in committee",
                              ...
                              "")) %>%
  select(`Reached Floor?`, Value, Briefing, Combined, `Cue-Taking`)
...
z2 <- lm(cosp ~ treatment.desk + bill + leg,
   data = subset(df , office.size > 1 &
                   !is.na(t.spill.desk) &
                   !is.na(t.spill.dist) &
                   !is.na(t.spill.ideo)),
...
z3 <- lm(cosp ~ treatment.dist + bill + leg,
   data = subset(df , office.size > 1 &
                   !is.na(t.spill.desk) &
                   !is.na(t.spill.dist) &
                   !is.na(t.spill.ideo)),
...
z4 <- lm(cosp ~ treatment.ideo + bill + leg,
         data = subset(df , office.size > 1 &
                         !is.na(t.spill.desk) &
                         !is.na(t.spill.dist) &
                         !is.na(t.spill.ideo)),
...
z1_ses <- z1$residuals[c('treatment1.0','treatment0.1','treatment1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z2_coef <- z2$coefficients[c('treatment.desk1.0','treatment.desk0.1','treatment.desk1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z2_ses <- z2$residuals[c('treatment.desk1.0','treatment.desk0.1','treatment.desk1.1')]%>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z3_coef <- z3$coefficients[c('treatment.dist1.0','treatment.dist0.1','treatment.dist1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z3_ses <- z3$residuals[c('treatment.dist1.0','treatment.dist0.1','treatment.dist1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z4_coef <- z4$coefficients[c('treatment.ideo1.0','treatment.ideo0.1','treatment.ideo1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z4_ses <- z4$residuals[c('treatment.ideo1.0','treatment.ideo0.1','treatment.ideo1.1')] %>% as_tibble() %>% mutate(value = value * 100) %>% round(2)

z_final <- z1_coef %>%
  mutate(value = c("Briefing", "Cue-Taking", "Combined"),
        `Offices` = z1_coef$value,
        `Office SE`= z1_ses$value,
        `Desks`= z2_coef$value,
        `Desks SE`= z2_ses$value,
